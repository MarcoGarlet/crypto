...
rshift = lambda x,y: ''.join([x[(i-y*1)%8] for i in range(8)])
int2vect = lambda c: bin(c%16).split('b')[1][::-1]+('0'*(4-len(bin(c%16).split('b')[1])))+bin(c//16%16).split('b')[1][::-1]+('0'*(4-len(bin(c//16%16).split('b')[1])))  
...
sbox=[''.join(x) for x in list(itertools.product([hex(x).split('x')[1] for x in range(16)],[hex(x).split('x')[1] for x in range(16)]))]
sbox=[sbox[i*16:i*16+16] for i in range(len(sbox)//16)]

cvector = int2vect(0x63)
cmatrix = ['10001111']+[rshift('10001111',i) for i in range(1,8)]
print(hex(gf_invert(0x95)))
